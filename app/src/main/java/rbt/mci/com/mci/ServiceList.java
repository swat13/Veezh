...
public class ServiceList extends Activity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final RSSFeed serListFeed = (RSSFeed) getIntent().getSerializableExtra("ServiceList");

        init();

        recView = (RecyclerView) findViewById(R.id.service_list);
        progress = (RelativeLayout) findViewById(R.id.progress_layout);
        recView.setLayoutManager(new LinearLayoutManager(ServiceList.this));
        recView.setItemAnimator(new DefaultItemAnimator());

        if (serListFeed != null) {
            ListAdapter serList_ad = new ListAdapter(serListFeed);
            recView.setAdapter(serList_ad);
        }
    }
    ...
    public class ListAdapter extends RecyclerView.Adapter<FeedViewHolder> {
        ...
        public void onBindViewHolder(FeedViewHolder holder, final int position) {
            ...
            Picasso.with(getApplicationContext())
                    .load("http://www.veezh.com/upload/service/" + image + ".jpg")
    ...
    private void init() {
        ...
        mPager.setAdapter(new SliderAdapter(ServiceList.this, ImagesArray));
    ...
    private class AsyncShowList extends AsyncTask<String, Void, RSSFeed> {
        ...
        protected RSSFeed doInBackground(String... params) {
            ...
            if (params[0].equals("D")) {
                result1 = myParser.getServiceDetail(id);
        ...
        protected void onPostExecute(RSSFeed result) {
            ...
            if (result1 != null) {
                if (result1.getItemCount() > 0) {
                    if (type.equals("D")) {
                        startActivity(new Intent(ServiceList.this, ServiceDetail.class).putExtra("serviceDetail", result1).putExtra("serviceImage", result2));
                    }
                }
            } else if (activeNetworkInfo == null) {
                Toast.makeText(ServiceList.this, "خطا در برقراری ارتباط", Toast.LENGTH_SHORT).show();
            }
        }
    }
}
